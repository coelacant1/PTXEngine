...
Vector2D Vector2D::RightPerpendicular() const {
    return Vector2D(Y, -X);
}
...
uc3d::UString Vector2D::ToString() const {
    uc3d::UString x = Mathematics::DoubleToCleanString(X);
    uc3d::UString y = Mathematics::DoubleToCleanString(Y);
