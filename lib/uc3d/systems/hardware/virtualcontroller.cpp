...
VirtualController::VirtualController(CameraManager* cams,
                                     ...
                                     const char* reg_name)
: Controller(cams),
  ctrl_name_(ctrl_name ? ctrl_name : "/uc3d_ctrl"),
  reg_name_(reg_name ? reg_name : "/uc3d_reg") {}
...
void VirtualController::Initialize() {
    ...
    (void)uc3d_registry_init(reg_, reg_name_.c_str(), camCount);
    ...
    for (uint32_t i = 0; i < camCount; ++i) {
        ...
        pc.ui_name  = namebuf;
        pc.fb_name  = std::string("/uc3d_fb")   + std::to_string(i);
        pc.geom_name= std::string("/uc3d_geom") + std::to_string(i);

        // Init FB (1xN) + shared control
        (void)uc3d_shm_init(pc.shm, pc.fb_name.c_str(), pc.W, pc.H, ctrl_name_.c_str());

        // Init Geometry: publish N XY pairs
        pc.geom.width  = logicalW;
        pc.geom.height = logicalH;
        (void)uc3d_geom_init(pc.geom, pc.geom_name.c_str(), pc.count);

        pc.rgb.resize(size_t(pc.count) * 3u, 0);
        pc.xy .resize(size_t(pc.count) * 2u, 0.f);

        // Record in registry
        uc3d_registry_set(reg_, i, pc.ui_name.c_str(), i, pc.count, logicalW, logicalH);

        cams_.push_back(std::move(pc));
    }

    uc3d_registry_publish(reg_, (uint32_t)(cams_.size()));
}
...
void VirtualController::Display() {
    ...
    for (uint32_t i = 0; i < n; ++i) {
        ...
        // Publish this camera
        uc3d_geom_publish(pc.geom, pc.xy.data(), pc.count);
        uc3d_publish_rgb888(pc.shm, pc.W, pc.H, pc.rgb.data());
    }
}
