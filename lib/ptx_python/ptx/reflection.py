...
def _resolve_library_path(
    ...
    # Fall back to look in a couple of repo-relative directories.
    repo_root = Path(__file__).resolve().parents[2]
    candidates = [
        repo_root / "build" / "libptx_c_api.so",
        repo_root / "build" / "lib" / "libptx_c_api.so",
        repo_root / "PTXEngine" / "build" / "libptx_c_api.so",
        repo_root / "PTXEngine" / "build" / "lib" / "libptx_c_api.so",
    ]
