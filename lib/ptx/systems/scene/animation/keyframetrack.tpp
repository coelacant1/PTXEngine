...
float KeyFrameTrack<maxParameters, maxKeyFrames>::GetCurrentTime(){
    currentTime = fmod(ptx::Time::Millis() / 1000.0f + timeOffset, stopFrameTime - startFrameTime) + startFrameTime;//normalize time and add offset
...
void KeyFrameTrack<maxParameters, maxKeyFrames>::SetCurrentTime(float setTime){
    float currentSecs = ptx::Time::Millis() / 1000.0f;
