...
void BoundaryMotionSimulator::Randomize(float range) {
    for (int i = 0; i < sphereCount; i++) {
        bS[i]->velocity.X = um3d::Random::Float(0.0f, 1.0f) * range - range / 2.0f;
        bS[i]->velocity.Y = um3d::Random::Float(0.0f, 1.0f) * range - range / 2.0f;
        bS[i]->velocity.Z = um3d::Random::Float(0.0f, 1.0f) * range - range / 2.0f;
    }
}

Vector3D BoundaryMotionSimulator::RandomRatio(float range) {
    Vector3D vRand;
    vRand.X = 1.0f + um3d::Random::Float(0.0f, 1.0f) * range;
    vRand.Y = 1.0f + um3d::Random::Float(0.0f, 1.0f) * range;
    vRand.Z = 1.0f + um3d::Random::Float(0.0f, 1.0f) * range;
    return vRand;
}
