...
ImageSequence::ImageSequence(Image* image, const uint8_t** data, unsigned int imageCount, float fps) {
    this->startTime = um3d::Time::Millis();
    this->image = image;
    this->data = data;
...
void ImageSequence::Reset() {
    startTime = um3d::Time::Millis();
}

void ImageSequence::Update() {
    float currentTime = fmod((um3d::Time::Millis() - startTime) / 1000.0f, frameTime) / frameTime; // Normalize time to ratio
