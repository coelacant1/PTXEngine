#include "../../../include/um3d/render/raster/rasterizer.hpp"

Quaternion Rasterizer::rayDirection;

RGBColor Rasterizer::RasterizePixelAccel(Triangle2D** triangles, int numTriangles, Vector2D pixelRay) {
...
RGBColor Rasterizer::RasterizePixel(Triangle2D** triangles, int numTriangles, Vector2D pixelRay) {
    float zBuffer = 3.402823466e+38f;
    int triangle = 0;
    bool didIntersect = false;
    float u = 0.0f, v = 0.0f, w = 0.0f;
...
void Rasterizer::Rasterize(Scene* scene, CameraBase* camera) {
    if (camera->Is2D()) {
        ...
        for (uint16_t i = 0; i < camera->GetPixelGroup()->GetPixelCount(); ++i) {
            ...
            RGBColor color = RasterizePixel(leafNode->GetEntities(), leafNode->GetCount(), pixelRay);
